...
def _run(cmd: list[str], job_id: str) -> None:
    """
    Run a yt-dlp command, stream progress to the job store, enforce timeout.
    A reader thread drains stdout while proc.wait() enforces the timeout.
    """
    ...
    try:
        ...
        proc.kill()
        proc.wait()
        raise RuntimeError("Download timed out after 600 seconds")
    finally:
